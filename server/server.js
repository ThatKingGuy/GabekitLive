...
function createSetFromString(name, str){
    console.log(str);
        var questions = [];
        var possibleAns = [];
        for(s in str.split(";")){
            if(str.split(";")[s].split(",")[1] != null){
                possibleAns.push(str.split(";")[s].split(",")[1]);
            }
        }

       
        for(s in str.split(";")){
            if(str.split(";")[s].split(",")[0] != null){
                var question = str.split(";")[s].split(",")[0];
                var correctAnswer = str.split(";")[s].split(",")[1];
                var answer1 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                while(answer1 === undefined){
                    answer1 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                }

                var answer2 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                while(answer2 === answer1 || answer2 === correctAnswer || answer2 === undefined){
                    answer2 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                }

                var answer3 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                while(answer3 === answer1 || answer3 === answer2 || answer3 === correctAnswer || answer3 === undefined){
                    answer3 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                }

                var answer4 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                while(answer4 === answer1 || answer4 === answer2 || answer4 === answer3 || answer4 === correctAnswer || answer4 === undefined){
                    answer4 = possibleAns[Math.floor(Math.random()*str.split(";").length)];
                }
                ...
                answers[correct-1] = correctAnswer;
                questions.push({"question": shuffle(question), "answers": answers, "correct": correct});
            }
        }
...
function clearDB(){
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db('kahootDB');
        dbo.collection('kahootGames').drop(function (err, delOK) {
            if (err) throw err;
...
server.listen(80, () => {
    var str = "los días de la semana,the days of the week;los días,the days;la semana,the week;lunes,Monday;martes,Tuesday;miércoles,Wednesday;jueves,Thursday;viernes,Friday;sábado,Saturday;domingo,Sunday;hoy,today;mañana,tomorrow;ayer,yesterday;el fin de la semana,the weekend;¿Qué día es?,What day is it?;Es,It's;¿Qué día es hoy?,What day is today?;¿Qué día es mañana?,What day is tomorrow?;¿Qué día fue ayer?,What day was yesterday?;¿Cuál es tu día favorito?,What is your favorite day?;";
    console.log("Server started on port 80");
    //clearDB();
    //createSetFromString("Spanish", str)
});
